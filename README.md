...
Open the sidebar → **"Try a sample day"** to demo the entire experience (extraction, valuation, MoSPI comparison, micro-business idea) with **zero API calls** — useful if you don't have a Gemini key handy yet.
...
By default, everything lives in Streamlit session state only. BigQuery persistence is opt-in, consent-gated, and only available in `gcp` mode.
